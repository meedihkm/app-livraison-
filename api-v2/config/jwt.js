...
if (process.env.NODE_ENV === 'production') {
  if (!process.env.JWT_SECRET || process.env.JWT_SECRET.length < 32) {
    console.error('ERREUR CRITIQUE: JWT_SECRET non dÃ©fini ou trop court en production!');
    process.exit(1);
  }
  if (!process.env.SUPER_ADMIN_KEY || process.env.SUPER_ADMIN_KEY.length < 32) {
    console.error('ERREUR CRITIQUE: SUPER_ADMIN_KEY non dÃ©fini ou trop court en production!');
    process.exit(1);
  }
}
...
module.exports = {
  jwtSecret: process.env.JWT_SECRET || (() => {
    ...
    console.warn('âš ï¸ Utilisation du secret JWT de dÃ©veloppement');
  ...
  superAdminKey: process.env.SUPER_ADMIN_KEY || (() => {
    ...
    console.warn('âš ï¸ Utilisation de la clÃ© super-admin de dÃ©veloppement');
