...
const getSSLConfig = () => {
  ...
  if (caPath) {
    try {
      ...
      logger.info("[DB] Certificat CA chargÃ© depuis:", caPath);
    } catch (err) {
      logger.error("[DB] Erreur lecture certificat CA:", err.message);
  ...
  if (!sslConfig.rejectUnauthorized) {
    logger.warn(
      "[SECURITY] DATABASE_SSL_REJECT_UNAUTHORIZED=false - Validation SSL dÃ©sactivÃ©e",
    );
    logger.warn(
      "[SECURITY] Ceci expose Ã  des attaques MITM - Configurer un certificat CA en production",
    );
  }
...
pool.on("connect", (client) => {
  if (process.env.NODE_ENV !== "production") {
    logger.info("[DB] Nouvelle connexion PostgreSQL Ã©tablie");
  }
  ...
  client
    ...
    .then((result) => {
      if (result.rows[0]?.ssl === "on") {
        if (process.env.NODE_ENV !== "production") {
          logger.info("[DB] Connexion SSL active");
        }
      }
    })
    .catch(() => {
...
pool.on("error", (err, client) => {
  logger.error("[DB] Erreur PostgreSQL inattendue:", {
    error: err.message,
    stack: err.stack,
  });
...
const testConnection = async () => {
  try {
    ...
    logger.info(`[DB] Connexion vÃ©rifiÃ©e - Base: ${result.rows[0].db}`);
    return true;
  } catch (err) {
    logger.error("[DB] Ã‰chec connexion:", err.message);
    return false;
  }
};
